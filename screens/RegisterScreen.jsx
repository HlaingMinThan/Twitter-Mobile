...
export default function Register({ navigation }) {
    ...
    return (
        <View style={{ justifyContent: 'center', alignItems: 'center', flex: 1 }}>
            ...
            <TextInput value={name} onChangeText={setName} style={tailwind('border w-80 my-3 border-gray-400 p-2')} placeholder='Name'></TextInput>
            {(error && error.name) && <Text style={tailwind('text-red-500')}>{error.name}</Text>}
            ...
            <Pressable style={tailwind('bg-blue-500 px-6 py-2 rounded-full')} onPress={() => register(name, username, email, password)}>
